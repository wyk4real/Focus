...
class Swin_G(nn.Module):
    def __init__(self, img_size=512, patch_size=4, in_c=1, embed_dim=128,
        ...
        self.connect_layers = nn.ModuleList([
            Reshape_Conv(input_resolution=self.patch_grid // (2 ** i_layer),
                         dim=int(embed_dim * 2 ** i_layer))
            for i_layer in range(len(depths))])
        # build metal neighborhood embedding layers
        self.emb_layers = nn.ModuleList([
            SwinTransformerBlock(dim=int(embed_dim * 2 ** i_layer),
                                 input_resolution=self.patch_grid // (2 ** i_layer),
                                 num_heads=num_heads[i_layer],
    ...
    def forward_features(self, x, metal):
        x = self.PatchEmbed(x)

        nbhd_embs = []
        for inx, layer in enumerate(self.emb_layers):
            if inx == 0:
                nbhd_emb = x
                emb = layer(nbhd_emb)
                metal = self.Max_Pool(self.Max_Pool(metal))
                metal_map = metal.flatten(2).transpose(1, 2) \
                            @ torch.ones((1, emb.shape[2]), dtype=torch.float32, device=metal.device)
                nbhd_embs.append(emb * metal_map)
            else:
                nbhd_emb = self.merging_layers[inx - 1](nbhd_emb)
                emb = layer(nbhd_emb)
                metal = self.Max_Pool(metal)
                metal_map = metal.flatten(2).transpose(1, 2) \
                            @ torch.ones((1, emb.shape[2]), dtype=torch.float32, device=metal.device)
                nbhd_embs.append(emb * metal_map)
    ...
    def forward(self, image, mask):
        ...
        cnt_x = self.forward_features(x=image, metal=mask)
