...
class Swin_G(nn.Module):
    def __init__(self, img_size=512, patch_size=4, in_c=1, embed_dim=128,
        ...
        self.connect_layers = nn.ModuleList([
            Reshape_Conv(input_resolution=self.patch_grid // (2 ** i_layer),
                         dim=int(embed_dim * 2 ** i_layer))
            for i_layer in range(len(depths))])
        # build metal self embedding layers
        self.emb_layers = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(in_channels=in_c,
                          out_channels=embed_dim * 2 ** i_layer,
                          kernel_size=patch_size * 2 ** i_layer,
                          stride=patch_size * 2 ** i_layer,
                          bias=False),
                nn.BatchNorm2d(embed_dim * 2 ** i_layer),
    ...
    def forward_features(self, x, metal):
        ...
        for inx, layer in enumerate(self.layers):
            self_emb = self.emb_layers[inx](metal)
            x = x + self_emb.flatten(2).transpose(1, 2)
            x = layer(x)
